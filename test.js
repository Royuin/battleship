...
test('test if gameboard addShip adds ship to coordinates horizontally', () => {
  ...
  expect(
    mockBoard.b[2] && mockBoard.c[2] && mockBoard.d[2] && mockBoard.e[2]
  ).toBeDefined();
});
...
test('gameboard addShip adds ship to coordinates vertically', () => {
  ...
  expect(
    mockBoard.d[1] && mockBoard.d[2] && mockBoard.d[3] && mockBoard.d[4]
  ).toBeDefined();
});
...
test('gameboard receive attack method changed value to miss if coordinates are incorrect', () => {
  ...
  expect(mockBoard.b[1]).toBe('miss');
});
...
test('computer taking turn after player takes their turn', () => {
  ...
  p2.attack = function () {
    let compX =
      p1.gameboard.row[Math.floor(Math.random() * p1.gameboard.row.length)];
